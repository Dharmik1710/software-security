...
DEBUG=True #FIXME make DEBUG=False if you do not want to use GDB to debug
...
if DEBUG:
    try:
        os.remove("./shellme_copy")
    except OSError:
        pass
    shutil.copyfile("shellme", "shellme_copy")
    st = os.stat('shellme_copy')
    os.chmod('shellme_copy', st.st_mode|stat.S_IEXEC)

    io = process('./shellme_copy')
else:
    io = process('./shellme')
...
offset = 150 #FIXME it should not be 150
